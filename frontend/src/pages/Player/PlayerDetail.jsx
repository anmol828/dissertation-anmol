    ...
    <div className="space-y-6">
      <section className="rounded-4xl border border-line bg-surface p-6 shadow-[0_16px_48px_rgba(15,23,42,0.06)] md:p-8">
        <div className="grid gap-8 lg:grid-cols-[auto_1fr_auto] lg:items-center">
          <img
            ...
            alt={playerName}
            className="h-36 w-36 rounded-3xl object-cover shadow-sm"
          />
          <div>
            <p className="text-xs uppercase tracking-[0.18em] text-pitch-strong">Player profile</p>
          ...
          <div className="flex flex-col gap-2">
            {canContact ? (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="rounded-2xl bg-pitch px-5 py-3 text-center text-sm font-medium text-white hover:bg-pitch-strong"
              >
                Contact on WhatsApp
              </a>
            ) : (
              <span className="rounded-2xl bg-surface-2 px-5 py-3 text-center text-sm text-muted">
                No contact added
              </span>
            )}
            <Link
              to="/search"
              className="rounded-2xl border border-line bg-surface px-5 py-3 text-center text-sm font-medium text-foreground hover:bg-surface-2"
            >
              Back to search
            </Link>
          </div>
      ...
      <section className="rounded-3xl border border-line bg-surface p-6 shadow-[0_12px_40px_rgba(15,23,42,0.06)]">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <p className="text-sm text-muted">City: {profile.city || "Not added"}</p>
          <p className="text-sm text-muted">Age: {profile.age}</p>
          <p className="text-sm text-muted">Preferred foot: {profile.preferredFoot}</p>
          <p className="text-sm text-muted">Jersey number: {profile.jerseyNumber || "Not added"}</p>
        </div>
      </section>
    </div>
  );
